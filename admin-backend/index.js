...
app.get("/", (req, res) => {
  res.redirect(
    `https://oauth2.sky.blackbaud.com/authorization?client_id=${CLIENT_ID}&response_type=code&redirect_uri=http://localhost:3000/oauth2/callback`
  );
});
...
const updateToken = (token) => {
  return new Promise((resolve, reject) => {
    ...
    collection
      .updateOne({ purpose: "oauth" }, { $set: { refresh_token: token } })
      .then(() => {
        resolve();
      })
      .catch((err) => {
